...
verifyButton.MouseButton1Click:Connect(function()
    ...
    if UserInputService.TouchEnabled then
        ...
        if UserInputService.KeyboardEnabled then
            -- Likely Android
            key = "rhx_hwid_" .. generateHWID()
            deviceType = "Android"
        else
            -- Likely iOS
            key = "rhx_" .. generateUUID()
            deviceType = "iOS"
        end
    end
    
    local url = validKeyURL .. key
    print("Validating key with URL: " .. url)
    
    -- HTTP request to validate the key
    ...
    if success then
        ...
        print("Error validating key: " .. response)
        game.StarterGui:SetCore("SendNotification", {
            Title = "Key System",
            Text = "Error validating key",
            Duration = 5,
        })
    end
end)
...
getKeyButton.MouseButton1Click:Connect(function()
    ...
    local url = "https://rhxkeysystem.vercel.app/generatekey-android.html"
    setclipboard(url)
    game.StarterGui:SetCore("SendNotification", {
        Title = "Key System",
