...
MakeWindow({
  Hub = {
    Title = "Riley Hub X | Murder Mystery 2",
    Animation = "By Riley"
  },
  Key = {
    KeySystem = false,
...
if not Welcome then Message("Success", "Enjoy!", 5) end
...
AddToggle(Main, {
  Name = "Auto Escape Murderer (Broken)",
...
local section = AddSection(Main, {"Freeze"})
...
local section = AddSection(Main, {"Teleport"})
...
function SpamChat()
			while autochat == true do
				local ohString1 = "Riley Hub X On Top"
...
local section = AddSection(Main, {"By Riley"})

local section = AddSection(Main, {"Youtube: RileyBeeOrion"})

local section = AddSection(Main, {"Best Murder Mystery 2 Script"})
